...
class GammaPoisson(GammaExponential):
    def update(self, *args):
        if len(args) == 1:
            return self.alpha + sum(args[0]), self.beta + len(args[0]), GammaPoisson(self.alpha + sum(args[0])), self.beta + len(args[0])
        elif len(args) == 2:
            return self.alpha + args[0], self.beta + args[1], GammaPoisson(self.alpha + args[0], self.beta + args[1])
        else:
            raise SyntaxError("Illegal number of arguments")
    def predict(self, x):
