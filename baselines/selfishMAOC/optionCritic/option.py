...
def createOptions(rng, env):
	...
	# mu_policy is the policy over options
	#mu_weights = dict.fromkeys(joint_state_list, dict.fromkeys(joint_option_list, 0))
	mu_policies = [SoftmaxPolicy(rng, len(env.cell_list), params['agent']['n_options'], params['policy']['temperature']) for _ in range(params['env']['n_agents'])]
	
	options = []
	for i in range(params['agent']['n_options']):
		# options.append(Option(i, SoftmaxActionPolicy(len(env.cell_list), len(env.agent_actions)), SigmoidTermination(
		# 	len(env.cell_list))))
		options.append(Option(i, SoftmaxPolicy(rng, len(env.cell_list), len(env.actions), params['policy']['temperature']), SigmoidTermination(rng,
			len(env.cell_list)))) 
