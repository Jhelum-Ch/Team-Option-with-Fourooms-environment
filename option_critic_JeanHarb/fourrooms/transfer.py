...
if __name__ == '__main__':
    ...
    parser.add_argument('--nagents', help='Number of agents', type=float, default=1)
    ...
    history = np.zeros((args.nruns, args.nepisodes))
    ...
    for run in range(args.nruns):
        ...
        # Make copies for each agent
        all_agent_option_policies = [option_policies for _ in range(args.nagents)]
        all_agent_option_terminations = [option_terminations for _ in range(args.nagents)]
        all_agent_policy = [policy for _ in range(args.nagents)]
        all_agent_critic = [critic for _ in range(args.nagents)]
        all_agent_action_weights = [action_weights for _ in range(args.nagents)]
        all_agent_action_critic = [action_critic for _ in range(args.nagents)]
        all_agent_termination_improvement = [termination_improvement for _ in range(args.nagents)]
        all_agent_intraoption_improvement = [intraoption_improvement for _ in range(args.nagents)]


        current_agent_states = np.zeros(args.nagents)
        all_agent_options = np.zeros(args.nagents)
        all_agent_actions = np.zeros(args.nagents)
        all_agent_cumreward = np.zeros(args.nagents)
        rewards = np.zeros(args.nagents)


        # for i in range(args.nagents):
        #     option_policies = all_agent_option_policies[i]
        #     option_terminations = all_agent_option_terminations[i]
        #     policy = all_agent_policy[i]
        #     critic = all_agent_critic[i]
        #     action_weights = all_agent_action_weights[i]
        #     action_critic = all_agent_action_critic[i]
        #     termination_improvement = all_agent_termination_improvement[i]
        #     intraoption_improvement = all_agent_intraoption_improvement[i] 
        ...
        for episode in range(args.nepisodes):
            ...
            for i in range(args.nagents):
                ...
                current_agent_states[i] = phi
                all_agent_options[i] = all_agent_policy[i].sample(phi)
                all_agent_actions[i] = all_agent_option_policies[i].sample(phi)
                all_agent_critic[i].start(phi,all_agent_options[i])
                all_agent_action_critic[i].start(phi,all_agent_options[i],all_agent_actions[i])
            ...
            for step in range(args.nsteps):
                for i in range(args.nagents):
                    ...
                    current_agent_states[i] = phi
                    rewards[i] = reward
                if numel(current_agent_states) ~= numel(set(current_agent_states)):
                    duplicate = dict((x, duplicates(List, x)) for x in set(List) if List.count(x) > 1)
                    rewards = [rewards[j] - params['env']['collision_penalty'] for j in list(duplicate.values()[0])]


                for i in range(args.nagents):
                    phi = current_agent_states[i]
                    reward = rewards[i]
                    option_policies = all_agent_option_policies[i]
                    option_terminations = all_agent_option_terminations[i]
                    policy = all_agent_policy[i]
                    ...
                    termination_improvement = all_agent_termination_improvement[i]
                    intraoption_improvement = all_agent_intraoption_improvement[i] 
                    cumreward = all_agent_cumreward[i]
            ...
            history[run, episode] = step
            # print('Run {} episode {} steps {} cumreward {} avg. duration {} switches {}'.format(run, episode, step, cumreward, avgduration, option_switches))
            print('Run {} episode {} steps {} cumreward {}'.format(run, episode, step, all_agent_cumreward))
        np.save(fname, history)
        dill.dump({'all_agent_intra_policies':all_agent_option_policies, 'all_agent_policy':all_agent_policy, 'all_agent_term':all_agent_option_terminations}, open('selfish-maoc-options.pl', 'wb'))
        print(fname)
